...
export function endCombatActions(bot,add,{go,bounded,item}){
 ...
 if(support?.boundingBox!=='block'&&item('obsidian')&&p.offset(0,1.62,0).distanceTo(pillar.position.offset(.5,.5,.5))<4.5){
  if([5,6,7].includes(d.metadata[15])||d.metadata[15]===3&&d.position.y<top+8)add('wait_safe_support','Wait behind the low pillar until the dragon leaves the support position',()=>bot.waitForTicks(30));
  else add('setup_bed_support',`Place one obsidian support east of the pillar at (1,${top},0) from low cover`,async()=>{await bot.equip(item('obsidian'),'hand');await bot.placeBlock(pillar,new Vec3(1,0,0));});return;
 }
 ...
 if(inPosition&&!unsafe(p)&&support?.boundingBox==='block'&&(bed||bedItem&&[2,3,5,6,7].includes(d.metadata[15])))add('one_timed_bed',`Arm ONE bed attack. Place its foot above the pillar and pillow east on obsidian. Wait up to 16 seconds for the observed dragon head within 4.3 blocks of the pillow while the dragon is at least Y${top+2}, then explode once from ground cover. Abort on nearby breath.`,async()=>{
   let placed=bot.findBlock({matching:b=>b.name.endsWith('_bed'),maxDistance:12});
   if(!placed){const clearStart=Date.now();while(preciseHead(d).distanceTo(pillar.position.offset(.5,1.5,.5))<4&&Date.now()-clearStart<5000)await bot.waitForTicks(1);const it=bot.inventory.items().find(i=>i.name.endsWith('_bed'));if(!it)throw new Error('No bed');await bot.equip(it,'hand');await bot.look(Math.PI*1.5,0,true);await bot.waitForTicks(2);if(bot.breathReflexActive||unsafe(bot.entity.position))throw Error('Breath interrupts bed placement');try{await bot._placeBlockWithOptions(pillar,new Vec3(0,1,0),{forceLook:'ignore',swingArm:'right'});}catch(e){if(!bot.blockAt(pillar.position.offset(0,1,0))?.name.endsWith('_bed'))throw e;}placed=bot.blockAt(pillar.position.offset(0,1,0));}
   await bot.lookAt(placed.position.offset(.5,.5,.5),true);await bot.waitForTicks(2);
   const start=Date.now();while(Date.now()-start<16000){const dragon=Object.values(bot.entities).find(e=>e.name==='ender_dragon');if(!dragon||dragon.metadata[8]<=0)return 'Dragon defeated';if(unsafe(bot.entity.position))throw new Error('Breath entered cover; escape next');if(bot.entity.position.distanceTo(stand.offset(.5,0,.5))>1)throw new Error('Lost cover position');if(!bot.blockAt(placed.position)?.name.endsWith('_bed'))throw new Error('Bed was destroyed');const head=preciseHead(dragon),pillow=new Vec3(1.5,top+1.5,.5);if(dragon.position.y>=top+2&&head.distanceTo(pillow)<4.3&&bot.entity.onGround&&Date.now()-lastBlast>700){const before=dragon.metadata[8];bot.emit('bedBlastAttempt',{head,body:dragon.position.clone(),phase:dragon.metadata[15],distance:head.distanceTo(pillow)});await bot.activateBlock(bot.blockAt(placed.position));lastBlast=Date.now();await bot.waitForTicks(12);return `One bed blast: dragon health ${before} -> ${dragon.metadata[8]}, head distance ${head.distanceTo(pillow).toFixed(2)}`;}await bot.waitForTicks(1);}return 'Bed armed; no valid head window yet';
 });
