...
export function endCombatActions(bot,add,{go,bounded,item}){
 ...
 if(inPosition&&!unsafe(p)&&support?.boundingBox==='block'&&(bed||bedItem&&[2,3,5,6,7].includes(d.metadata[15])))add('one_timed_bed',`Arm ONE bed attack. Place the bed above the natural pillar, with its pillow east. Wait up to 16 seconds for the observed dragon head within 4.3 blocks of the pillow while the dragon is at least Y${top+2}, then explode once from ground cover. Abort on nearby breath.`,async()=>{
   let placed=bot.findBlock({matching:b=>b.name.endsWith('_bed'),maxDistance:12});if(placed&&placed.getProperties().facing!=='east'){await bot.dig(placed,true);placed=null;}
